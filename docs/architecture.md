...
-   **HomePage** --- pantalla principal con las tarjetas de recetas,
    filtros, buscador, orden y receta aleatoria\
...
-   **FavoriteButton** --- botón para guardar o quitar una receta de
    favoritas\
-   **HomeSkeleton** --- nuevo: skeleton para la carga inicial de la
    HomePage\
...
-   **Context API** --- gestiona las recetas favoritas y permite
    sincronizar el estado entre `RecipeCard`, `RecipeDetailPage` y los
    hooks
...
Maneja: - carga de una receta por ID\
- loading\
- error\
...
## Endpoints de la API
  ...
  DELETE              /api/v1/favoritas/:id               Elimina una receta de
                                                          favoritas
  -------------------------------------------------------------------------------------
...
-   recetas servidas desde `recetas.json`\
-   favoritas almacenadas en memoria
...
-   El frontend hace peticiones a la API\
-   La API lee `recetas.json` y gestiona favoritas\
-   El frontend actualiza la UI según la respuesta
